...
def _call_llm(prompt: str, model: str = WORKER_MODEL) -> str:
    ...
    return complete(prompt, task="node_factory", max_tokens=1024)
...
def _validate_new_node(classification: dict, db) -> list[str]:
    ...
    # エッジなしノード生成は拒否（設計: 自分でG2ギャップを作らない）
...
def generate_nodes(dry_run: bool = False, verbose: bool = True,
                   max_new_nodes: int = 5, max_evidence: int = 3) -> dict:
    ...
    # 1サイクル最大20ノード（設計: new_nodes: 20/cycle）
    MAX_NEW_NODES = max(0, max_new_nodes)
    generated_this_cycle = 0

    for item in unresolved[:max(0, max_evidence)]:
        ...
        for i, claim in enumerate(claims):
            if (claim.get("review_required") or claim.get("attribution") != "own_result"
                    or claim.get("confidence") == "speculative"):
                continue
            if generated_this_cycle >= MAX_NEW_NODES:
                break
            ...
            # 帰属先ありでも信頼度が低い場合はLLM分類に進む（閾値0.50）
            if attr["node_id"] is not None and attr.get("confidence", 0) >= 0.50:
                continue  # 高信頼の帰属先あり → skip
            ...
            if decision in ("new_instance", "new_class"):
                ...
                if not dry_run:
                    ...
                    if parent_rev:
                        rationale = "[HYP_EDGE] " + nn.get("differentiation", "node_factory auto-generated")
...
def check_promotion(db, node_id: str, revision: int = 1) -> dict:
    ...
    from knowledge_policy import claim_usable, edge_usable
    claim_ids = [r[0] for r in db.execute('SELECT claim_id FROM claim WHERE node_id=? AND node_revision=?', (node_id, revision))]
    audited = bool(claim_ids) and all(claim_usable(db, cid, require_node=False) for cid in claim_ids)
    edge_count = sum(edge_usable(db, r[0]) for r in db.execute(
        'SELECT edge_id FROM edge WHERE from_node=? OR to_node=?', (node_id, node_id)))
    eligible = bool(eligible and audited and edge_count >= 2)
    reason_parts = []
    if not audited:
        reason_parts.append('current claim validation required')
