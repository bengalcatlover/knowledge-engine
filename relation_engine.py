...
2段ロケット方式:
  ...
  Stage 2: LLM検証 → evidence-backed か HYP_EDGE 隔離

機能:
  1. generate_candidates() — co-evidenceペアから関係候補を生成
  2. verify_candidate()   — LLMで候補を検証、エッジ型を決定
...
def _call_llm(prompt: str, model: str = WORKER_MODEL) -> str:
    ...
    return complete(prompt, task="relation_engine", max_tokens=1024)
...
def verify_candidate(db: sqlite3.Connection, candidate: EdgeCandidate,
    ...
    # Model confidence alone is not independent evidence for a new relation.
...
def resolve_g3_gaps(db: sqlite3.Connection, dry_run: bool = False, verbose: bool = True) -> dict:
    ...
    for gap in gaps:
        ...
        stats["checked"] += 1
        from knowledge_policy import edge_usable
        if not edge_usable(db,edge_id):
            stats['unresolvable'] += 1
            continue  # co-citation is a candidate signal, never relation validation
        ...
        if co_evidence:
            ...
            if not dry_run:
                # Keep the validated edge fingerprint intact; co-evidence adds no proof.
                # G3を解消
                db.execute(
