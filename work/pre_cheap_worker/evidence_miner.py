...
パイプライン:
  ...
  2. LLM(Haiku)でabstractから原子的claim抽出（最大5件/abstract）
...
def fetch_abstract(doi: str) -> dict:
    """Abstract取得: Semantic Scholar → OpenAlex フォールバック"""
    ...
    return {"ok": False, "doi": doi, "reason": "no abstract from S2 or OpenAlex"}
...
def check_duplicate(new_claim_text: str, existing_claims: list[dict]) -> dict:
    ...
    # Fable設計: 0.92以上 = identical, 0.75-0.92 = similar（gray zone）, <0.75 = new
    ...
    for i, claim in enumerate(claims):
        ...
        if not dry_run:
            ...
            add_support(db, evidence_id,
                        ...
                        target_claim=cid,
                        support_role="premise",
                        rationale=f"evidence_miner extracted (attribution={attribution['confidence']:.2f})",
                        assessor="evidence_miner")
...
def mine_evidence(evidence_id: str, dry_run: bool = False, verbose: bool = True) -> dict:
    ...
    if verbose:
        print(f"  [2/4] Extracting claims from abstract ({len(abstract)} chars)")
...
def mine_all(dry_run: bool = False, verbose: bool = True) -> dict:
